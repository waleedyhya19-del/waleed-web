...
interface ModeratorDetailProps {
  user: User;
}

function ModeratorStat({
  label,
  icon,
  value,
}: {
  ...
  return (
    <div className="surface-card-muted rounded-[1.35rem] p-4">
      <div className="flex items-center gap-3">
        <div className="flex size-10 items-center justify-center rounded-2xl bg-cyan-500/10 text-cyan-600">
          {icon}
        </div>
        <div>
...
const ALL_ROLE_OPTIONS = [Role.END_USER, Role.MODERATOR, Role.ADMIN];

export function ModeratorDetail({ user }: ModeratorDetailProps) {
  ...
  const { data: adminGuard } = useAsyncResource(
    async () => usersApi.list({ role: Role.ADMIN, take: 2 }),
    [],
    {
      cacheKey: 'users:admins:guard',
      staleTimeMs: 30_000,
    }
  );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      invalidateResourceCacheByPrefix(`moderators:detail:${record.id}`);
      invalidateResourceCache('users:admins:guard');
      successToast(t('moderators.deleteSuccess'));
  ...
  return (
    <div className="page-grid">
      <section className="surface-card border-0 p-5 sm:p-6">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="section-kicker">{t('moderators.detail')}</p>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          ...
          <ModeratorStat
            ...
            icon={<KeyRound className="size-4" />}
            value={record.preferredLanguage}
          />
        </div>
      </section>
      ...
      {isEditing ? (
        <section className="surface-card border-0 p-5 sm:p-6">
          <div className="mb-5">
            <p className="section-kicker">{t('moderators.edit')}</p>
      ...
      <ConfirmationDialog
        ...
        title={t('moderators.delete')}
        description={t('moderators.deleteConfirm')}
