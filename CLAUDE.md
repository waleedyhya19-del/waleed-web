...
Admin dashboard for **Telephoney Mafqud** (تليفوني) — a lost/stolen phone reporting platform. This is the `web/` directory of a monorepo. The backend lives in `api/` (NestJS). This frontend consumes the NestJS REST API directly via a fetch-based client; there is no BFF or separate backend.
